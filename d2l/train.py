...
__all__ = ['evaluate_loss', 'train_ch10', 'train_2d','evaluate_accuracy', 'squared_loss', 
'grad_clipping', 'sgd', 'train_and_predict_rnn', 'train_ch3', 'train_ch5','MaskedSoftmaxCELoss',
'train_ch7', 'translate_ch7', 'to_onehot' , 'predict_rnn', 'train_and_predict_rnn_nn', 
'predict_rnn_nn', 'grad_clipping_nn', 'train_model', 'evaluate_model']
...
def train_model(model, loss, optimizer, train_iter, test_iter, num_epochs, device):
    train_loss_sum, train_acc_sum = [], []
    test_loss_sum, test_acc_sum = [], []
    for epoch in range(num_epochs):
        train_loss, train_acc, num_data = 0, 0, 0
        ...
        train_loss_sum.append(train_loss / num_data)
        train_loss_sum.append(train_acc / num_data)
        print(f"Epoch {epoch}: training loss {train_loss / num_data: .3f}, "
              f"train accuracy {train_acc / num_data * 100: .3f}%")

        test_loss, test_acc = evaluate_model(model, loss, test_iter, device)
        test_loss_sum.append(test_loss)
        test_acc_sum.append(test_acc)
        print(f"Epoch {epoch}: testing loss {test_loss: .3f}, "
              f"test accuracy {test_acc * 100: .3f}%")

    return train_loss_sum, train_acc_sum, test_loss_sum, test_acc_sum

def evaluate_model(model, loss, test_iter, device):
    model.eval()
    num_data, test_loss, test_acc = 0, 0, 0
    ...
    return test_loss / num_data, test_acc / num_data
