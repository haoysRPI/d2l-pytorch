...
class PIKACHU(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        loc_chw = np.zeros((4,))
        loc_chw[0] = (loc[0] + loc[2])/2    # cx
        loc_chw[1] = (loc[1] + loc[3])/2    # cy
...
def intersect(box_a: torch.Tensor, box_b: torch.Tensor) -> float:
    ...
    # the min of the lower-right corner
    min_xy = torch.max(box_a[:, None, :2], box_b[None, :, :2])
    # the max of the upper-left corner
...
def MultiBoxPrior(feature_map_sizes, sizes, aspect_ratios):
    ...
    height, width = feature_map_sizes   
    ...
    for h, w in itertools.product(range(height), range(width)):
        ...
        for s in sizes:
            boxes.append((cx, cy, s * math.sqrt(aspect_ratios[0]), s / math.sqrt(aspect_ratios[0])))
...
def MultiBoxTarget(class_true, bb_true, anchors):
    ...
    class_true +=1    
    class_target = torch.zeros(anchors.shape[0])
